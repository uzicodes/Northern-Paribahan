...
import { Bus, MapPin, Users, Wifi, Wind, Shield, ChevronRight, Loader2, Star } from 'lucide-react';

// Static bus data with images mapped to public/bus/ folder
const busShowcase = [
    {
        image: '/bus/volvo.png',
        brand: 'Volvo',
...
export default function BusesPage() {
    ...
    useEffect(() => {
        const fetchBuses = async () => {
            try {
                ...
                // silently fail, show static cards
            } finally {
                setLoading(false);
            }
        };
    ...
    const cards = busShowcase.map((showcase, index) => {
        ...
        return {
            ...
            totalSeats: matched?.seats?.length || 0,
            ...
            <div className="relative overflow-hidden" style={{ backgroundColor: '#172144' }}>
                ...
                <div className="relative max-w-6xl mx-auto px-4 py-16 text-center">
                    ...
                    <p className="text-lg text-slate-300 max-w-2xl mx-auto">
                        Choose from our collection of modern, well-maintained buses for a comfortable journey across Bangladesh.
                    </p>
                </div>
            </div>
                ...
                {loading ? (
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                        {cards.map((bus, index) => (
                            <div
                                key={index}
                                className="group bg-white rounded-3xl overflow-hidden shadow-sm border border-slate-100 hover:shadow-2xl hover:-translate-y-2 transition-all duration-500 cursor-pointer"
                                onClick={() => bus.id && router.push(`/buses/${bus.id}`)}
                            ...
                            >
                                ...
                                <div className={`relative h-52 bg-gradient-to-br ${bus.color} overflow-hidden`}>
                                    ...
                                    <div className="absolute top-4 right-4 flex items-center gap-1 bg-white/90 backdrop-blur-sm px-2.5 py-1 rounded-full shadow-lg">
                                        ...
                                        <span className="text-sm font-bold text-slate-700">{bus.rating}</span>
                                    </div>

                                    {/* Type Badge */}
                                    <div className="absolute top-4 left-4 px-3 py-1 rounded-full bg-white/90 backdrop-blur-sm text-xs font-bold text-slate-700 shadow-lg uppercase tracking-wider">
                                        {bus.type}
                                    </div>
                                </div>
                                ...
                                <div className="p-5">
                                    ...
                                    <div className="flex items-center justify-between pt-3 border-t border-slate-100">
                                        ...
                                        {/* CTA Arrow */}
                                        <div className="w-9 h-9 rounded-full bg-indigo-600 text-white flex items-center justify-center group-hover:bg-indigo-700 group-hover:scale-110 transition-all duration-300 shadow-md shadow-indigo-200">
