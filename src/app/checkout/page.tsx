...
function CheckoutContent() {
    ...
    // Listen for payment popup messages
    useEffect(() => {
        const handleMessage = (event: MessageEvent) => {
            // Verify origin in production if needed
            if (event.data?.type === 'PAYMENT_SUCCESS') {
                clearCheckoutSession();
                router.push(`/booking/success?tran_id=${event.data.tran_id}`);
            } else if (event.data?.type === 'PAYMENT_FAILED') {
                router.push(`/booking/failed?tran_id=${event.data.tran_id || 'unknown'}&reason=${event.data.reason || 'Unknown'}`);
            }
        };
